...
# 데이터 확인
path = "./250604_snsd_data_labeling_2000_Into_The_New_World_0513_02_only_comments_english.csv"
df = pd.read_csv(path, encoding = "utf-8")
data_X = list(df['comment'].values)
...
# 학습용 및 검증용 데이터셋 분리 (scikit learn에 있는 tranin_test_split 함수 사용, random_state 는 반드시 일치시킬 것)
...
# MobileBERT에 영화 리뷰 데이터를 Finetuning하기 위한 데이터 설정
...
# 학습(loss), 검증(train accuracy, validation accuracy)
epoch_results = []
...
for e in range(epochs):
    ...
    for batch in tqdm(train_dataloader, desc=f"Evalucation Train Epoch {e+1}", leave=True):
        batch_ids, batch_mask, batch_labels = batch
...
# 학습 종료후 epoch별 학습 경과 및 검증 정확도 출력
for idx, (loss, train_acc, val_acc) in enumerate(epoch_results, start=1):
    print(f"Epoch {idx}: Train loss: {loss:.4f}, Trian Accuracy: {train_acc:.4f}, Validation Accuracy: {val_acc:.4f}")
